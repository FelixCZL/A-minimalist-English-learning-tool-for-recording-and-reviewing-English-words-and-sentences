...
Keep the response strictly as JSON. Make the definition simple and practical. Focus on tech/business contexts."""
    ...
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are an English learning assistant. Always respond with valid JSON only."},
...
Keep the response strictly as JSON. Focus on what makes this sentence useful for learning and how the pattern can be reused."""
    ...
    try:
        response = client.chat.completions.create(
            model="deepseek-chat",
            messages=[
                {"role": "system", "content": "You are an English learning assistant focusing on sentence patterns and expressions. Always respond with valid JSON only."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.7,
            max_tokens=800
...
def analyze_content(content: str, source: str = None, note: str = None) -> Dict[str, Any]:
    """
    分析内容（单词或句子）
    返回结构：
    {
